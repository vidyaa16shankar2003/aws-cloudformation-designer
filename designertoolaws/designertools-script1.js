  ...
  return [
    ...
    },
    {
      ...
      attributes: {
        ...
        d: "m -10 -2 l 18 0 l 0 -5 l 11 7 l -11 7 l 0 -5 l -18 0 z",
        fill: "#4666E5",
        stroke: "none",
        "stroke-width": 2,
        "pointer-events": "none",
  ...
  arrowOptions.forEach((arrowOption) => {
    ...
    arrowItem.addEventListener("click", () => {
      ...
      setTimeout(() => {
        ...
        let gElement = document.querySelector('g.marker-arrowhead-group-target')
        let pathElement = gElement.querySelector('path.marker-arrowhead')
        var newPathData = "M 10 7 L 0 13 L 10 19 z";
        // Function to prevent further changes to the d attribute
        function preventPathDataChange(event) {
          console.log("DOM modified")
          event.preventDefault();
        }
  
        // Add an event listener to the path element to prevent attribute changes
        pathElement.addEventListener("DOMAttrModified", preventPathDataChange, false);
  
        // Set the 'd' attribute of the path element with the new path data
        pathElement.setAttribute("d", newPathData);

      }, 1);
      
      console.log(pathElement)
      // Define the new path data
...
function createLinkLabelInput(linkView, x, y) {
  ...
  input.style.top = y*zoomLevel - 20 + "px";
...
// joint.shapes.standard.define('custom.arrowhead', {
//   markup: '<path class="marker-arrowhead" end="target" d="M 10 7 L 0 13 L 10 19 z"/>',
// });
...
function applyArrowType(x, y, sourceView, targetView, selectedArrowType) {
  ...
  const markerTargetD = "M 10 7 L 0 13 L 10 19 z";
  ...
  const link = new joint.shapes.standard.Link({
    ...
    // arrowhead: { to: { type: 'custom.arrowhead' } },
  });
  ...
  console.log(targetView.model.id)
  graph.addCell(link);
  // console.log(link)
  updateHistory();
...
const arrowOptions = [
  {
    id: "arrow-type-1",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${inputWidth}" height="${inputHeight}" viewBox="0 0 41 20" fill="none"><path d="M31.5 2C31.5 2 33.5 6.66667 39.5 10C33.5 13.3333 31.5 18 31.5 18" stroke="black" stroke-width="1" ></path><path d="M39.5 10H28.5" stroke="black" stroke-width="1"></path><path d="M2 10L33 10" stroke="black" stroke-width="1" ></path></svg>`,
    name: "right-arrow",
  },
  {
    id: "arrow-type-2",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${inputWidth}" height="${inputHeight}" viewBox="0 0 41 20" fill="none"><path d="M31.5 2C31.5 2 33.5 6.66667 39.5 10C33.5 13.3333 31.5 18 31.5 18" stroke="black" stroke-width="1" ></path><path d="M39.5 10H28.5" stroke="black" stroke-width="1"></path><path d="M2 10L33 10" stroke="black" stroke-width="1" stroke-dasharray="4 4 "></path></svg>`,
    name: "dotted-arrow",
  },
  {
    id: "arrow-type-3",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${inputWidth}" height="${inputHeight}" viewBox="0 0 46 20" fill="none"><path d="M36.5 2C36.5 2 38.5 6.66667 44.5 10C38.5 13.3333 36.5 18 36.5 18" stroke="black" stroke-width="1" ></path><path d="M44.5 10H33.5" stroke="black" stroke-width="1"></path><path d="M11 10L38 10" stroke="black" stroke-width="1"></path><path d="M9.5 18C9.5 18 7.5 13.3333 1.5 10C7.5 6.66667 9.5 2 9.5 2" stroke="black" stroke-width="1" ></path><path d="M1.5 10L12.5 10" stroke="black" stroke-width="1"></path></svg>`,
    name: "doubleside-arrow",
  },
  {
    id: "arrow-type-4",
    svg: `<svg xmlns="http://www.w3.org/2000/svg" width="${inputWidth}" height="${inputHeight}" viewBox="0 0 46 20" fill="none"><path d="M36.5 2C36.5 2 38.5 6.66667 44.5 10C38.5 13.3333 36.5 18 36.5 18" stroke="black" stroke-width="1" ></path><path d="M44.5 10H33.5" stroke="black" stroke-width="1"></path><path d="M11 10L38 10" stroke="black" stroke-width="1" stroke-dasharray="4 4 "></path><path d="M9.5 18C9.5 18 7.5 13.3333 1.5 10C7.5 6.66667 9.5 2 9.5 2" stroke="black" stroke-width="1" ></path><path d="M1.5 10L12.5 10" stroke="black" stroke-width="1"></path></svg>`,
...
const ResizeTool = elementTools.Control.extend({
  ...
  function createGroup(position, size) {
    const imageUrl="images/ungroup.svg";
  ...
  function groupSelectedElements() {
    if (selectedElements.length > 1) {
      for (let i=0; i<selectedElements.length; i++){
        xarray.push(selectedElements[i].attributes.position.x);
        yarray.push(selectedElements[i].attributes.position.y);
      }
      ...
      const group = createGroup(
        { x: Math.min(...xarray) - 40 / zoomLevel, y: Math.min(...yarray) - 30 / zoomLevel},
        {
          width: Math.max(...xarray) - Math.min(...xarray) + 140,
          height: Math.max(...yarray) - Math.min(...yarray) + 160,
        }
      );
  ...
  paper.el.addEventListener("pointermove", (e) => {
    if (isDragging) {
      ...
      allembededelements = [...allembededelements, ...elementsInSelection];

      if (selectedElements.length > 0) {
        if (paperContainer.scrollLeft == 0){
          groupButton.style.left = startX + "px";
        }
        else {
          groupButton.style.left = startX + paperContainer.scrollLeft + "px";
        }
        if (paperContainer.scrollTop == 0){
          groupButton.style.top = startY + "px";
        }
        else{
          groupButton.style.top = startY + paperContainer.scrollTop + "px";
        }
...
var zoomLevel = 1;
var paperWidthPercentage = 100; 
var paperHeightPercentage = 100; 
  ...
  function displayResults(results) {
      ...
      if (results.length < 0) {
          ...
          $.each(results, function (index, item) {
            ...
            var img = document.createElement('img');
            img.src = imagesUrl+'/AWS/' + item.path;
            img.width = '30';
            img.height = '30';
            img.draggable = true;
            img.name = item.name;
...
//       // scrollToRight();
//     } 
//     else if (x + elementWidth > (paperWidth)) {
//       newX = paperWidth - elementWidth;
//       // newX = 30;
//       // scrollToLeft();
//     }
...
//       // scrollToBottom();
//     } 
//     else if (y + elementHeight + 75 > paperHeight) {
//       newY = paperHeight - elementHeight - 75;
//       // newY = 0;
//       // scrollToTop();
//     }
...
//               if (embedslist[k] == currentgraphjson.cells[j].id) {
//                 // console.log(k)
//                 // console.log(positionjsonarray)
//                 if (positionjsonarray[0].cells[j].type != "link") {
...
// const currentViewBox = document.getElementById("current-view-box");
// const miniMap = document.getElementById("mini-map");
// const viewportIndicator = document.getElementById("viewport-indicator");

// paperContainer.addEventListener("scroll", () => {
//     // Update the position and size of the viewport indicator in the mini-map
...
//     const containerHeight = paperContainer.clientHeight;

//     // Calculate the corresponding position and size in the mini-map
//     const miniMapX = (scrollLeft / (paperContainer.scrollWidth - containerWidth)) * miniMap.clientWidth;
//     const miniMapY = (scrollTop / (paperContainer.scrollHeight - containerHeight)) * miniMap.clientHeight;
//     const miniMapWidth = (containerWidth / paperContainer.scrollWidth) * miniMap.clientWidth;
//     const miniMapHeight = (containerHeight / paperContainer.scrollHeight) * miniMap.clientHeight;

//     // Set the position and size of the viewport indicator in the mini-map
//     viewportIndicator.style.transform = `translate(${miniMapX}px, ${miniMapY}px)`;
//     viewportIndicator.style.width = miniMapWidth + "px";
//     viewportIndicator.style.height = miniMapHeight + "px";
// });
...
// function updateCurrentViewBox() {
//   // Calculate the bounding box of all elements in the graph
...
//   // Set the current view box based on the calculated bounding box
//   currentViewBox.style.left = minX + 'px';
//   currentViewBox.style.top = minY + 'px';
//   currentViewBox.style.width = (maxX - minX) + 'px';
//   currentViewBox.style.height = (maxY - minY) + 'px';
// }

// // Listen for changes in the paper, including element additions, deletions, position changes, etc.
// graph.on('change', () => {
//   // Update the mini-map
//   updateMiniMap();
...
graph.on('change:position', function(cell) {
  if (cell.isElement()) {
    // console.log(cell.id)
    for (let i = 0; i < allembededelements.length; i++){
      if (allembededelements[i].id == cell.id){
        let graphjson = graph.toJSON();
        for (let j = 0; j < graphjson.cells.length; j++){
          if (graphjson.cells[j].id == cell.attributes.parent){
            let parentcell = graph.getCell(cell.attributes.parent);
            if (cell.attributes.position.x < (graphjson.cells[j].position.x + 20)){
              parentcell.set('position', {x: cell.attributes.position.x - 20, y:graphjson.cells[j].position.y});
              parentcell.set('size', {width: graphjson.cells[j].size.width + 20, height:graphjson.cells[j].size.height});
            }
            if (cell.attributes.position.x > (graphjson.cells[j].position.x + graphjson.cells[j].size.width - 120)){
              parentcell.set('size', {width: graphjson.cells[j].size.width + (cell.attributes.position.x - (graphjson.cells[j].position.x + graphjson.cells[j].size.width - 120)), height: graphjson.cells[j].size.height});
            }
            if (cell.attributes.position.y < (graphjson.cells[j].position.y + 30)){
              parentcell.set('position', {x: graphjson.cells[j].position.x, y:cell.attributes.position.y - 30});
              parentcell.set('size', {width:graphjson.cells[j].size.width , height:graphjson.cells[j].size.height + 20});
            }
            if (cell.attributes.position.y > (graphjson.cells[j].position.y + graphjson.cells[j].size.height - 130)){
              parentcell.set('size', {width: graphjson.cells[j].size.width, height: graphjson.cells[j].size.height + (cell.attributes.position.y - (graphjson.cells[j].position.y + graphjson.cells[j].size.height - 130))});
            }
            // console.log(cell.attributes.position)
            // console.log(graphjson.cells[j])
          }
        }
      }
    }
